...
async function getAuthorizedBookmark(id: string) {
  ...
  const bookmark = await prisma.bookmark.findUnique({ where: { id, userId: session.user.id } })
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if ('unauthorized' in check) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if ('notFound' in check) return NextResponse.json({ error: 'Bookmark not found' }, { status: 404 })
